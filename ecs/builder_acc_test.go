...
func TestBuilderAcc_validateRegion(t *testing.T) {
	...
	err := access.Config()
	if err != nil {
		t.Fatalf("init ApsaraStackAccessConfig failed: %s", err)
	}
...
func checkImageDisksSettings() builderT.TestCheckFunc {
	return func(artifacts []packer.Artifact) error {
		...
		describeSnapshotRequest.Headers = map[string]string{"RegionId": "cn-wulan-env82-d01"}
		describeSnapshotRequest.QueryParams = map[string]string{"AccessKeySecret": "EuKRGrBTs7ZUyg2AvsmQ7OrwXQrkTq", "Product": "ecs", "Department": "11", "ResourceGroup": "27"}
...
func checkRegionCopy(regions []string) builderT.TestCheckFunc {
	return func(artifacts []packer.Artifact) error {
		...
		for r := range artifact.ApsaraStackImages {
			if r == "cn-beijing" {
				delete(regionSet, r)
				continue
			}
...
func TestBuilderAcc_forceDelete(t *testing.T) {
	t.Parallel()
	// Build the same ApsaraStack image twice, with ecs_image_force_delete on the second run
	builderT.Test(t, builderT.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
...
func checkECSImageSharing(uid string) builderT.TestCheckFunc {
	return func(artifacts []packer.Artifact) error {
		...
		describeImageShareRequest.ImageId = artifact.ApsaraStackImages["cn-beijing"]
...
func TestBuilderAcc_forceDeleteSnapshot(t *testing.T) {
	...
	// Build the same ApsaraStack image name twice, with force_delete_snapshot on the second run
	builderT.Test(t, builderT.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
